...
struct avl_node *rightRotate(struct avl_node *N){
    ...
    struct avl_node *new_root = N->left; // get new root

    N->height = max(height(N->right), height(new_root->right)) + 1; // N's new height is max height of right subtree or left->right subtree + 1

    new_root->parent = N->parent; // assign new root N's parent before updating N
    N->parent = new_root; // update N
...
struct avl_node *leftRotate(struct avl_node *N){
    ...
    struct avl_node *new_root = N->right;

    N->height = max(height(N->left), height(new_root->left)) + 1;
...
struct avl_node *_avl_balance(struct avl_node *root)
{

    if(root->left){
        if( height(root->left) > height(root->right) && height(root->left->left) > height(root->left->right) ){ // left-left

            //printf("rebalancing - left-left\n");

            return rightRotate(root);

        }
        else if( height(root->left) > height(root->right) && height(root->left->left) < height(root->left->right) ){ // left-right

            //printf("rebalancing - left-right\n");
    ...
    if(root->right){
        if( height(root->left) < height(root->right) && height(root->right->left) > height(root->right->right) ){ // right-left

            //printf("rebalancing - right-left\n");

            root->right = rightRotate(root->right);
            return leftRotate(root);

        } 
        else if( height(root->left) < height(root->right) && height(root->right->left) < height(root->right->right) ){ // right-right

            //printf("rebalancing - right-right\n");
...
struct avl_node* _avl_subtree_insert(struct avl_node* node, int key){
    ...
    if(height(node->left) - height(node->right) == 2 || height(node->left) - height(node->right) == -2){ node = _avl_balance(node); }
