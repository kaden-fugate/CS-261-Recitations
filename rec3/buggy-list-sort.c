...
void bubble_sort(struct link* head) {
  int swapped = 0;
  do {
    swapped = 0; // swapped false, cur set to head
    struct link* curr = head;
    while (curr->next != NULL) { // iterate to end of list <---- bug was here
      if (curr->val > curr->next->val) { // if val greater than next val, swap nodes
        swap(&curr->val, &curr->next->val);
        swapped = 1; // swap true
      }
      curr = curr->next;
    }
  } while (swapped); // repeat while vals are swapped
}
