...

{
   ...
   * Allocate space for the new array.
   */
  da->data = realloc(da->data, new_capacity * sizeof(void*));

  da->capacity = new_capacity; // change cap
}
...
void dynarray_insert(struct dynarray* da, void* val) {
  ...
  if (da->size == da->capacity) {
    _dynarray_resize(da, da->capacity + 1); // for method 3 & 4
    //_dynarray_resize(da, 2 * da->capacity); // for method 1 & 2
  }
